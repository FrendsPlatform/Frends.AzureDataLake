...
using Frends.AzureDataLake.DeleteFiles.Helpers;
...
public static class AzureDataLake
{
    ...
    /// <returns>Object { bool Success, List&lt;string&gt; DeletedFiles, Error Error }</returns>
    ...
    )
    {
        try
        {
            ...
            await Parallel.ForEachAsync(
                matches.Files,
                async (match, cancellationToken) =>
                {
                    var deletedFilePath = await DeleteFile(match.Path, container, cancellationToken);
                    parallelResults.Add(deletedFilePath);
                }
            );

            return new Result { Success = true, DeletedFiles = parallelResults.ToList() };
        }
        catch (Exception ex)
        {
