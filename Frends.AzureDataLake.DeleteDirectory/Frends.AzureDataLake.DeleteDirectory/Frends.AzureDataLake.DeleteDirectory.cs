...
public static class AzureDataLake
{
    ...
    /// <returns>Object { bool Success, bool DirectoryWasDeleted, string Message, Error Error }</returns>
    public static async Task<Result> DeleteDirectory([PropertyTab] Input input, [PropertyTab] Options options, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return new Result { Success = true, DirectoryWasDeleted = result, Message = "Directory deleted successfully." };
        }
        catch (Exception ex)
        {
