...
public static class AzureDataLake
{
    ...
    public static async Task<Result> DeleteDirectory([PropertyTab] Input input, [PropertyTab] Options options, CancellationToken cancellationToken)
    {
        ...
        try
        {
            DataLakeDirectoryClient directory = GetDataLakeDirectory(input);

            if (!await directory.ExistsAsync(cancellationToken) && !options.ThrowErrorIfDirectoryDoesNotExists)
    ...
    public static DataLakeDirectoryClient GetDataLakeDirectory(Input input)
    {
        try
        {
